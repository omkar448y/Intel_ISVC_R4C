...
public void select_ship_to_contact() throws InterruptedException {
	Thread.sleep(5000);
	driver.findElement(By.xpath("//label[text()='Ship To Contact']")).click();
	Thread.sleep(4000);

	driver.findElement(By.xpath("//button[@name='Ship To Conatct']/../..//lightning-base-combobox-item[@role='option']")).click();
...
//---------->25k----------------
	...
	public void select_remedy_credit_and_reason_ssr() throws InterruptedException {
		...
		// If SSR present then will add Code 	
	}
...
//-------------ULT Bulk Upload
	...
	public void upload_bulk_file_ult(String string) throws AWTException, InterruptedException {
		...
		upload.delay(2000);
		StringSelection stringSelection = new StringSelection(
				"C:\\Users\\oyadavx\\OneDrive - Intel Corporation\\Desktop\\ULT Bulk Upload Sheet\\ULT_BulkUploadTemplate (3).csv");
		Toolkit.getDefaultToolkit().getSystemClipboard().setContents(stringSelection, null);
		upload.keyPress(KeyEvent.VK_CONTROL); // Key Press On key Board
		Thread.sleep(2000);
		upload.keyPress(KeyEvent.VK_V);
...
Thread.sleep(10000);
	...
	public void select_service_type_sr_and_click_on_create_buttons_and_enter_sold_to(String string) throws InterruptedException {
...
public void validate_the_status() {
    ...
    List<WebElement> elements = driver.findElements(By.xpath("//span[@class='slds-grid slds-grid_align-center slds-truncate']"));
...
public Steps() {
    try {
        FileInputStream fis = new FileInputStream("C:\\Users\\oyadavx\\git\\Intel_ISVC_R4C\\Intel_ISVC_R4C\\src\\test\\java\\Config.properties");
        prop.load(fis);
    } catch (IOException e) {
        e.printStackTrace();
    }
...
public void check_status_open_unsubmitted() {
...
public void select_service_type_technical_and_click_on_create_buttons_and_enter_sold_through_excel() throws InterruptedException, IOException {
	contacts = new ContactPage_R4C_CaseCreation(driver);
	Thread.sleep(5000);
	contacts.clickoncreatebutton();
	Technical = new ContactPage_Technical(driver);
...
public void print_case_no_into_console() {
	Technical = new ContactPage_Technical(driver);
